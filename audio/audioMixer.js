...
const PRELOAD_TIMEOUT_MS = 15000;  // 15s — generous timeout for music files
...
export class AudioMixer {
   ...
   */
  /**
   * Synchronously create Audio elements for every file in the manifest.
   * CRITICAL: call this BEFORE unlockAudio() so unlock can prime all
   * elements within the user gesture window. Without this, Act 1's music
   * bed silently queues because its Audio element wasn't primed.
   */
  prepareElements(manifest) {
    if (!manifest || !Array.isArray(manifest)) return;
    for (const item of manifest) {
      if (!item.file || !item.category) continue;
      if (this._audioElements.has(item.file)) continue;
      const url = `${this.basePaths[item.category] || ''}${item.file}`;
      const audio = new Audio(url);
      audio.preload = 'auto';
      audio.muted = this.muted;
      this._audioElements.set(item.file, audio);
    }
  }
  ...
  async _preloadOne({ category, file }) {
    ...
    // REUSE the pre-created element if prepareElements() ran first.
    // This element was already primed by unlockAudio() and is ready to
    // play without further user gestures.
    let audio = this._audioElements.get(file);
    if (!audio) {
      audio = new Audio(url);
      audio.preload = 'auto';
      audio.muted = this.muted;
    ...
    // If element is already loaded enough, return immediately.
    ...
    return new Promise((resolve) => {
      ...
      const done = (success) => {
        ...
        if (success) {
          ...
          // Don't remove from _audioElements — it may still load eventually
          // and the element is at least primed for playback.
          this._failedFiles.add(file);
          this.onWarn(`Failed to load (within ${PRELOAD_TIMEOUT_MS}ms): ${url}`);
        }
        resolve(success);
      };
