...
const Act6GameplayMontage = React.memo(function Act6GameplayMontage({
  ...
  // Wire per-act audio: looks up Act 6 cues from welcomeModalAudioConfig,
  // calls mixer.enterAct on mount, mixer.update on every tick, mixer.exitAct
  // on unmount. No-op if AudioMixerContext is missing.
      ...
      <svg className="f4-svg" viewBox="0 0 480 1040" preserveAspectRatio="xMidYMid slice">
        ...
        <g style={{
          ...
          {/* Cast shadow under the truck (visible at cab/wheels area).
              Matches the canonical HTML reference exactly. */}
          <ellipse cx="240" cy="320" rx="78" ry="14" fill="#000" opacity="0.42" />
          {/* The truck PNG itself.
              Matches the canonical HTML reference (welcome_modal_FULL_sequence)
              EXACTLY: x=170, y=-156, width=140, height=525, no rotation.

              CRITICAL: this rendering EXPECTS a pre-rotated PNG, where the
              cab is at the BOTTOM of the image and the rear of the box is
              at the TOP. The animation puts the top of the image at the
              dock door (so the rear backs into the dock).

              If your source PNG is in standard orientation (cab at top,
              box at bottom), you MUST flip it 180° in any image editor
              before placing it in public/welcome_modal_assets/. Save it
